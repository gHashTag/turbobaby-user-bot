...
(async () => {
  ...
  // Trigger file upload via JS eval (simulate file selection)
  await page.evaluate(() => {
  ...
  console.log('Video elements after upload:', videos);
  
  const uploadLogs = logs.filter(l => l.includes('ADMIN RS') || l.includes('TEST'));
  uploadLogs.forEach(l => console.log('LOG:', l));
