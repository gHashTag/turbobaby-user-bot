...
(
  ...
  find . -type f -not -name "*.br" -not -name "*.gz" -print0 | while IFS= read -r -d '' f; do
    ...
    if command -v brotli >/dev/null 2>&1 && [ ! -f "${base}.br" ]; then
      ...
      # any .br here was written by the brotli call just made (and trunk's
      # `clean_final` empties dist/ before every build), so a timestamp has
      # nothing to prove. This test used to also demand that the .br not be
      # older than its source: the Windows brotli build (1.2.0, mingw64) copies
      # the source mtime rounded down to the whole second, bash compares with
      # sub-second precision, so every fresh .br read as older, the size check
      # never ran, and a 22-byte version.txt.br for a 17-byte version.txt was
      # left behind by every Windows build.
